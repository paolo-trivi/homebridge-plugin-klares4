...
- Boots and reconnects no longer publish placeholder states. Every login re-reads the panel configuration, and each re-read device replaced the known one with parser defaults, so HomeKit and Matter briefly saw lights off, covers closed, open windows closed, 0 °C sensors and 20 °C thermostats before the real state arrived — enough to fire automations. A re-read device now keeps its observed state, and at boot the state cached by Homebridge stays until the panel reports one.
- Dimmers are exposed on Matter as `DimmableLight`. Discovery precedes the first output status, so every light was registered as an on/off `OnOffLight` without level control. A light that reports a level is now re-registered once as `DimmableLight` under the same ID, and later boots register it that way straight away.
- An explicit Matter thermostat recovery no longer registers the same device twice. A status update arriving while the recovery removed the old endpoint started a second registration; Homebridge rejected one of them and the recovery ended on the `TemperatureSensor` fallback while consuming the request.
- A Matter endpoint is removed before it is registered again under the same ID: in the fallback after a registration that never became queryable, in every registration retry, and after each unregister (renames, recovery, prune). Homebridge 2.4 answers state reads as "absent" while it is still closing an endpoint, but keeps the ID reserved until the close finishes; a register issued in that window was rejected as "already registered", and a rename then took about 30 seconds and a retry to apply.
- Thermostat setpoints are written to the season the panel is actually in. After a restart, a setpoint on a thermostat in summer mode rewrote the unchanged summer value while the panel acknowledged the write, so the change was lost; and a `cool` command that the panel had rejected could switch the thermostat to summer on the next setpoint. The season now comes from the most recent of the panel's realtime report and the last acknowledged write, falling back to the cached configuration.
- A PIN typed as a number in `config.json` is masked in the log, in the debug capture and in telemetry. It is now converted to a string once at startup, and the log mask also recognises unquoted numbers.
- `customNames` survives the Homebridge UI. It was declared as per-category maps keyed by panel ID, which the UI silently drops when it rewrites `config.json`, taking every custom name — and the MQTT topics and Matter names derived from it — with it. The UI now edits an array of `{ deviceId, name }` rows; the legacy map form is still read, and a KSA import with `applyCustomNames` writes the array form.
- Saving the plugin settings from the Homebridge UI no longer hides every `matterOverrides` device from Matter. The "Exposed on Matter" checkbox had no default, so the UI wrote `exposed: false` on every override row and those devices were pruned from Matter after a few restarts. The checkbox is now ticked by default, and devices hidden by an override are listed in a startup warning so that configurations already affected can be spotted and fixed.
- The debug capture file reports the running plugin version instead of a stale `1.1.9-beta0`.
